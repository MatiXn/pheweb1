...
import {
  IcoEuro, IcoLightning, IcoChart, IcoBell, IcoFactory, IcoCheck,
  IcoCheckCircle, IcoUpload,
        ...
        <section style={{ background: '#ffffff', padding: '100px 28px' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(290px, 1fr))', gap: 20 }}>
              {[
                { icon: <IcoEuro />,       title: 'Erfolgsbasierte Provision', desc: 'Sie verdienen ~32,5 % des ersten Jahresgehalts bei erfolgreicher Einstellung — kein Fixkostenrisiko, keine monatliche Gebühr.' },
                { icon: <IcoLightning />,  title: 'Automatisches Matching', desc: 'Kandidaten hochladen, fertig. pheweb matcht sie automatisch mit offenen Unternehmensstellen — 24/7, ohne Ihren Aufwand.' },
                { icon: <IcoChart />,      title: 'Pipeline-Dashboard', desc: 'Alle Kandidaten, Matches, Statusupdates und Interessensbekundungen auf einem Blick. Kein Excel, keine E-Mail-Suche.' },
                { icon: <IcoBell />,       title: 'Echtzeit-Benachrichtigungen', desc: 'Sobald ein Unternehmen Interesse bekundet, werden Sie sofort informiert. Kein wochenlang warten, keine Reaktionszeit-Probleme.' },
                { icon: <IcoFactory />,    title: 'Fokussierter Markt', desc: 'Elektro, TGA, SHK, Mechatronik — genau da, wo der Fachkräftemangel am stärksten ist und die Nachfrage am höchsten.' },
                { icon: <IcoCheck />,      title: 'Strukturierter Closing-Prozess', desc: 'Von der Einwilligung bis zur Hiring-Bestätigung — alles dokumentiert auf der Plattform. Revisionssicher, nachvollziehbar.' },
              ].map((b, i) => (
        ...
        <section style={{ background: '#f8fafc', padding: '100px 28px' }}>
          <div style={{ maxWidth: 860, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                { n: '01', icon: <IcoCheckCircle />, title: 'Registrieren & verifizieren', desc: 'Konto anlegen, kurze Prüfung durchlaufen. Nach Freischaltung haben Sie sofortigen Zugang zum Dashboard.' },
                { n: '02', icon: <IcoUpload />,      title: 'Kandidaten hochladen', desc: 'Berufsbezeichnung, Skills, Erfahrung, Standort, Verfügbarkeit und Gehaltsvorstellung eintragen. Einzeln oder in Batch.' },
                { n: '03', icon: <IcoLightning />,   title: 'Automatisches Matching', desc: 'pheweb matcht Ihre Kandidaten mit offenen Unternehmensstellen. Bei Interesse erhalten Sie sofort eine Benachrichtigung.' },
                { n: '04', icon: <IcoEuro />,        title: 'Einwilligung einholen & Provision kassieren', desc: 'Sie holen die Kandidaten-Einwilligung ein, koordinieren das Gespräch — und erhalten bei erfolgreicher Einstellung Ihre Provision.' },
              ].map((s, i) => (
          ...
          <div style={{ maxWidth: 720, margin: '0 auto' }}>
            <div ref={pricing.ref} style={{ opacity: pricing.inView ? 1 : 0, transform: pricing.inView ? 'none' : 'translateY(30px)', transition: 'all 0.6s ease' }}>
              <div style={{ background: 'linear-gradient(135deg, #ecfdf5, #d1fae5)', border: '1px solid #a7f3d0', borderRadius: 24, padding: '56px 48px', textAlign: 'center' }}>
                <div style={{ fontSize: 40, marginBottom: 16, color: '#059669' }}><IcoEuro /></div>
